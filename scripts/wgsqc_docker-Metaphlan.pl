...
##Step2: conda activate metaphlan && perl wgsqc_docker-Step2.pl
...
system "mkdir $Outdir/02.Structure/Metaphlan/sams $Outdir/02.Structure/Metaphlan/bowtie2 $Outdir/02.Structure/Metaphlan/profiles\n";
...
system "cp $Outdir/02.Structure/Metaphlan/profiles/$Keyname.profile.tsv $Outdir/WGS_Report/$Keyname.metaphlan.report.txt\n";
...
#	unless(-e "$target.1.bt2"){system "/root/miniconda3/envs/metaphlan/bin/bowtie2-build $target $target\n";}
#	if(defined $fq2){system "/root/miniconda3/envs/metaphlan/bin/bowtie2 -1 $fq1 -2 $fq2 -x $target -S $Outdir/03.Target/$Keyname.nohuman.sam --no-unal\n";}else{system "/root/miniconda3/envs/metaphlan/bin/bowtie2 -U $fq1  -x $target -S $Outdir/03.Target/$Keyname.nohuman.sam --no-unal\n";}
...
#
#	   open(O1,">$Outdir/WGS_Report/$Keyname.nohuman.R1.fq");
...
#
#	if(defined $fq2){
#		open(O2,">$Outdir/WGS_Report/$Keyname.nohuman.R2.fq");
#	open(FILE,$fq2);
#	while(1){
#			my $line=<FILE>;
#			unless($line){last;}
