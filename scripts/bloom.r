...
p<-ggplot(data=otu,aes(x=factor(Time,levels=sort(unique(Time))),y=Percentage,group=People))+geom_line(aes(color=People),size=1.5)+geom_point(aes(color=People),size=4)
ggsave(paste(outpath,"_line.pdf", sep=""), p, width = 18, height = 10)